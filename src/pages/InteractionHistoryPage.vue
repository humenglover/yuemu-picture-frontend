    ...
    <div class="history-header">
      <div class="header-left">
        <div class="title-wrapper">
          <h2>{{ $t('pages.interactionHistoryPage.title') }}</h2>
          <div class="divider"></div>
          <div class="total-count">
            {{ $t('pages.interactionHistoryPage.totalRecords').split('{total}')[0] }} <span class="highlight">{{ pagination.total || 0 }}</span> {{ $t('pages.interactionHistoryPage.totalRecords').split('{total}')[1] }}
          </div>
        </div>
      </div>
      <div class="header-right">
        <a-button class="back-btn" @click="router.back()">
          ...
          {{ $t('pages.interactionHistoryPage.return') }}
        </a-button>
      </div>
    </div>
    ...
    <div class="type-selector">
      <div class="type-filter">
        <a-radio-group v-model:value="activeType" size="middle" button-style="solid">
          <a-radio-button value="received">{{ $t('pages.interactionHistoryPage.received') }}</a-radio-button>
          <a-radio-button value="sent">{{ $t('pages.interactionHistoryPage.sent') }}</a-radio-button>
        </a-radio-group>
      </div>
      ...
      <div class="stats-cards">
        ...
        >
          ...
          <div class="stat-info">
            <div class="stat-label">{{ $t('pages.interactionHistoryPage.commentHistory') }}</div>
          </div>
        </div>
        ...
        >
          ...
          <div class="stat-info">
            <div class="stat-label">{{ $t('pages.interactionHistoryPage.likeHistory') }}</div>
          </div>
        </div>
        ...
        >
          ...
          <div class="stat-info">
            <div class="stat-label">{{ $t('pages.interactionHistoryPage.shareHistory') }}</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="list-header">
        <div class="header-content">
          <div class="title-section">
            <span class="list-title">{{ $t('pages.interactionHistoryPage.listTitle').replace('{type}', getTypeName()) }}</span>
            <span class="list-count">{{ $t('pages.interactionHistoryPage.listCount').replace('{total}', pagination.total || 0) }}</span>
          </div>
          <div class="filter-tags">
            ...
            >
              <PictureOutlined />{{ $t('pages.interactionHistoryPage.picture') }}
            </a-tag>
            <a-tag
            ...
            >
              <FileTextOutlined />{{ $t('pages.interactionHistoryPage.post') }}
            </a-tag>
          </div>
        </div>
      </div>
      <div class="list-content">
        ...
        <template v-else>
          <div v-if="historyData.length === 0" class="empty-state">
            <a-empty :description="(activeType === 'received' ? $t('pages.interactionHistoryPage.emptyDescReceived') : $t('pages.interactionHistoryPage.emptyDescSent')) + getTypeName()" />
          </div>
          <template v-else>
            <UnreadCommentList
        ...
        <template v-if="historyData.length > 0">
          ...
          <div class="mobile-pagination" v-else>
            <div class="pagination-info">
              <span>{{ $t('pages.interactionHistoryPage.paginationTotal').replace('{total}', pagination.total) }}</span>
              <a-select
                v-model:value="pagination.pageSize"
                :options="pagination.pageSizeOptions.map(size => ({ value: Number(size), label: $t('pages.interactionHistoryPage.paginationSize').replace('{size}', size) }))"
            ...
            <div class="pagination-wrapper">
              ...
              />
              <div class="jump-page">
                <span>{{ $t('pages.interactionHistoryPage.jumpTo') }}</span>
                <a-input-number
                  ...
                  size="small"
                />
                <span>{{ $t('pages.interactionHistoryPage.page') }}</span>
              </div>
            </div>
          </div>
        </template>
...
const pagination = ref({
  ...
  showTotal: (total: number) => $t('pages.interactionHistoryPage.paginationTotal').replace('{total}', String(total)),
...
const fetchHistoryData = async () => {
  ...
  try {
    ...
    message.error(t('pages.interactionHistoryPage.errGetHistory'))
  } finally {
    loading.value = false
...
const getTypeName = () => {
  switch (interactionType.value) {
    case 'comments':
      return t('pages.interactionHistoryPage.types.comment');
    case 'likes':
      return t('pages.interactionHistoryPage.types.like');
    case 'shares':
      return t('pages.interactionHistoryPage.types.share');
    default:
      return t('pages.interactionHistoryPage.types.interaction');
  }
}
...
const handleJumpPage = () => {
  ...
  if (jumpPage.value < 1 || jumpPage.value > maxPage) {
    message.warning(t('pages.interactionHistoryPage.pageWarning').replace('{max}', String(maxPage)))
    return
  }
