...
const handleAlbumClick = (album: TimeAlbum) => { if (album.isPublic === 0) { selectedPrivateAlbum.value = album; showPasswordModal.value = true; nextTick(() => passwordInputRef.value?.focus()) } else { router.push({ name: 'TimeAlbumDetail', params: { id: album.id }, query: { isOwner: props.isOwner } }) } }
...
const handlePasswordSubmit = async () => { if (!selectedPrivateAlbum.value) return; try { const res = await getTimeAlbumByIdUsingGet({ id: selectedPrivateAlbum.value.id, password: passwordInput.value }); if (res.data.code === 0) { sessionStorage.setItem(`album_${selectedPrivateAlbum.value.id}_password`, passwordInput.value); router.push({ name: 'TimeAlbumDetail', params: { id: selectedPrivateAlbum.value.id }, query: { isOwner: props.isOwner } }); showPasswordModal.value = false; passwordInput.value = ''; selectedPrivateAlbum.value = null } else { message.error(t('components.timeAlbum.wrongPassword')); passwordInput.value = ''; nextTick(() => passwordInputRef.value?.focus()) } } catch (error) { message.error(t('components.timeAlbum.verificationFailed')) } }
