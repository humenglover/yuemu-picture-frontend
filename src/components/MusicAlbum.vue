...
const handleAlbumClick = (album: MusicAlbum) => { if (album.isPublic === 0) { selectedPrivateAlbum.value = album; showPasswordModal.value = true; nextTick(() => { passwordInputRef.value?.focus() }) } else { router.push({ name: 'MusicAlbumDetail', params: { id: album.id }, query: { isOwner: props.isOwner } }) } }
...
const handlePasswordSubmit = async () => { if (!selectedPrivateAlbum.value) return; try { const res = await fetchMusicAlbumByIdUsingGet({ id: selectedPrivateAlbum.value.id, password: passwordInput.value }); if (res.data.code === 0) { sessionStorage.setItem(`album_${selectedPrivateAlbum.value.id}_password`, passwordInput.value); router.push({ name: 'MusicAlbumDetail', params: { id: selectedPrivateAlbum.value.id }, query: { isOwner: props.isOwner } }); showPasswordModal.value = false; passwordInput.value = ''; selectedPrivateAlbum.value = null } else { message.error(t('components.musicAlbum.passwordError')); passwordInput.value = ''; nextTick(() => { passwordInputRef.value?.focus() }) } } catch (error: any) { message.error(error?.response?.data?.message || t('components.musicAlbum.validationFailed')) } }
